...
const KirimMotor: React.FC = () => {
    ...
    const [isActionMotor, setIsActionMotor] = useState(false)
    ...
    const [isNoPolisi, setIsNoPolisi] = useState('')
    const [isHargaBarang, setIsHargaBarang] = useState('')
    const [isAsuransiBarang, setIsAsuransiBarang] = useState(false)
    const [isPackingBarang, setIsPackingBarang] = useState(true)
    const [isAsuransiPrice, setIsAsuransiPrice] = useState(Number)
    const [isPackingPrice, setIsPackingPrice] = useState(Number)
    ...
    const [isCcBarang, setIsCcBarang] = useState('')
    const [isBeratBarang, setIsBeratBarang] = useState(Number)
    const [isJenisBarang, setIsJenisBarang] = useState('')
    ...
    function inputBarang(type:any, val:any){
        ...
        if(type === 'Jenis'){
            setIsJenisBarang(String(val))
        }
        if(type === 'CC'){
            setIsBeratBarang(Number(val))
            if(Number(val) === 100){
                setIsCcBarang('< 150.CC')
            }else{
                setIsCcBarang('> 150.CC')
            }
            setIsActionMotor(false)
        }
    ...
    function doCalculateBerat(){
        if(isNamaPengirim !== '' && isNamaPenerima !== '' && isHargaBarang !== '' && isCcBarang !== '' && isJenisBarang !== '' && isNoPolisi !== ''){
            setIsNamaBarang(`${isJenisBarang}, No Polisi: ${isNoPolisi}`)
            setIsFull(true)
        }else{
            setIsFull(false)
        }
    ...
    function getVoucher(){
        ...
        data.append('layanan' , 'Kirim Motor')
        $.ajax({
            type: "POST",
    ...
    function doOverview(){
        doClickToggle('Packing', isPackingBarang)
        let data = new FormData()
        data.append('user_id', String(localStorage.getItem('userid')))
        ...
        data.append('berat', String(isBeratBarang))
        data.append('layanan', 'Kirim Motor')
        ...
        if(isSVCPengirim !== '' && isSVCPenerima !== '' && isHUBPengirim !== '' && isHUBPenerima !== '' && isLokasiPengirim !== '' && isLokasiPenerima !== '' && String(isBeratBarang) !== ''){
            $.ajax({
                type: "POST",
                ...
                success:function(r:any){
                    ...
                    if(r.code === 200){
                        if(r.isValidVoucher === false){
                            ...
                            showToast('Tidak ada voucher yang digunakan', 'medium', 1200, 'middle')
                        }
                        setDataOverview(r)
            ...
            showToast('Masih ada data yang belum diisi', 'danger', 1200, 'top')
        }
    }
    ...
    function openModalLokasi(){
        ...
        data.append('type', String((isSelectInput === 'Pengirim')?'asal':'tujuan'))
    ...
    function doSelectAddress(id:any, lokasi:any, svc_id:any, hub_id:any, phone:any, name:any, alamat:any){
        if(svc_id !== '0' || hub_id !== '0'){
            if(isSelectInput === 'Pengirim'){
                if(svc_id === String(localStorage.getItem('svc_id'))){
                    $("ion-input[name='lokasiPengirim']").val(lokasi)
                    ...
                    showToast('Alamat tidak sesuai dengan service center kamu', 'danger', 1200, 'middle')
                }
            }else{
                $("ion-input[name='lokasiPenerima']").val(lokasi)
    ...
    function doClickToggle(type:any, val:Boolean){
        var asuransi = Number(isHargaBarang) * (1/100)
        var kubikasi = (50*50*50) / 1000000
        var packing = Number(kubikasi * 600000)
        if(type === 'Asuransi'){
            setIsAsuransiBarang(Boolean(val))
            if(Boolean(val) === true){
                setIsAsuransiPrice(Number(asuransi))
            }else{
                setIsAsuransiPrice(0)
            }
        }
        if(type === 'Packing'){
            setIsPackingBarang(Boolean(val))
            if(Boolean(val) === true){
                setIsPackingPrice(Number(packing))
            }else{
                setIsPackingPrice(0)
            }
        }
    ...
    function doOrder(){
        if(isAgree){ 
            ...
            data.append('lokasiPengirim', isLokasiPengirim)
            data.append('lokasiPenerima', isLokasiPenerima)
            data.append('svcSource', isSVCPengirim)
            data.append('svcDest', isSVCPenerima)
            data.append('hubSource', isHUBPengirim)
            data.append('hubDest', isHUBPenerima)
            ...
            data.append('totalBerat', String(isBeratBarang))
            data.append('totalHarga', String(dataOverview['price_after_voucher']))
            ...
            data.append('price_packing', String(isPackingPrice))
            data.append('price_asuransi', String(isAsuransiPrice))
            

            $.ajax({
                type: "POST",
                url: "https://xpdcargo.id/login/CallbackV2/addKirimMotor",
                data: data,
                processData: false,
                contentType: false,
                dataType: "json",
    ...
    function BTPrint(){
        ...
        const cmdstest = [
            'SIZE 70 mm,80 mm',
            'CLS',
            `TEXT 0, 10, "2", 0, 1, 1, "INFORMASI BARANG (Kirim Motor)"`,
            `TEXT 0, 39, "1", 0, 1, 1, "${isNamaBarang}"`,
            `TEXT 0, 85, "1", 0, 1, 1, "QTY 1"`,
            `TEXT 0, 110, "2", 0, 1, 1, "---------------------------------------"`,
    ...
    return(
        <IonPage>
                ...
                <IonGrid id='Reguler'>
                    <IonRow>
                        <IonCol size='12'>
                            <IonText mode='ios'>
                                Informasi Motor
                                <IonIcon icon={(isHargaBarang === '' || isCcBarang === '' || isNoPolisi === '' || isJenisBarang === '')?alertCircleOutline:checkmarkCircleOutline} color={(isHargaBarang === '' || isCcBarang === '' || isNoPolisi === '' || isJenisBarang === '')?'danger':'success'} />
                            </IonText>
                        </IonCol>
                        
                        <IonCol size='12' style={{background:"rgba(108,122,137,0.15)", padding:"1px 5px 1px 5px", margin:"3px 0", borderRadius:"10px", display:"flex"}}>
                            <IonInput mode='ios' inputMode='text' placeholder='Jenis Motor' style={{textAlign:"start", fontSize:"12px", width:"50vw"}} value={isJenisBarang} onIonChange={(e)=>{inputBarang('Jenis', String(e.detail.value))}} />
                            <IonInput mode='ios' inputMode='text' placeholder='No. Polisi' style={{textAlign:"start", fontSize:"12px", width:"50vw"}} value={isNoPolisi} onIonChange={(e)=>{inputBarang('No. Polisi', String(e.detail.value))}} />
                        </IonCol>
                        <IonCol size='12' style={{background:"rgba(108,122,137,0.15)", padding:"1px 5px 1px 5px", margin:"3px 0", borderRadius:"10px"}}>
                            <IonInput mode='ios' inputMode='text' placeholder='Bersaran CC' style={{textAlign:"start", fontSize:"12px"}} value={isCcBarang} readonly={true} onClick={()=>{setIsActionMotor(true)}} />
                        </IonCol>
                        ...
                        <IonCol size='5' style={{display:'flex', flexDirection:"column"}}>
                            <IonText mode='ios' style={{display:"flex", flexDirection:"row", justifyContent:"start", margin:"2px 0"}}>
                                <IonToggle mode='ios' checked={isAsuransiBarang} onIonChange={(e)=>{doClickToggle('Asuransi', Boolean(e.detail.checked))}}></IonToggle>
                                <span style={{fontSize:"12px", margin:"8px 2px"}} >Asuransi</span>
                            </IonText>
                            <IonText mode='ios' style={{display:"flex", flexDirection:"row", justifyContent:"start", margin:"2px 0"}}>
                                <IonToggle mode='ios' checked={isPackingBarang} onIonChange={(e)=>{doClickToggle('Packing', Boolean(e.detail.checked))}}></IonToggle>
                ...
                <IonContent fullscreen>
                    ...
                    <IonGrid id='Reguler'>
                        <IonRow>
                            ...
                            <IonCol size='3' style={{textAlign:"start", padding:"1px 5px"}}>
                                <IonText mode='ios' style={{fontSize:"12px"}} color='dark'>
                                    Jenis Motor
                                </IonText>
                            </IonCol>
                            <IonCol size='9' style={{textAlign:"end", padding:"1px 5px"}}>
                                <IonText mode='ios' style={{fontSize:"12px"}} color='dark'>
                                    {isNamaBarang}
                                </IonText>
                            </IonCol>
                    ...
                    <IonGrid id='Reguler'>
                        <IonRow>
                            ...
                            <IonCol size='6' style={{textAlign:"end", padding:"1px 5px"}}>
                                <IonText mode='ios' style={{fontSize:"12px"}} color='dark'>
                                    {(isAsuransiBarang === true && isPackingBarang === false)?`Rp${(Number(dataOverview['price_after_voucher']) + isAsuransiPrice).toLocaleString(undefined, {maximumFractionDigits:2})}`:''}
                                    {(isPackingBarang === true && isAsuransiBarang === false)?`Rp${(Number(dataOverview['price_after_voucher']) + isPackingPrice).toLocaleString(undefined, {maximumFractionDigits:2})}`:''}
                                    {(isPackingBarang === true && isAsuransiBarang === true)?`Rp${(Number(dataOverview['price_after_voucher']) + isPackingPrice + isAsuransiPrice).toLocaleString(undefined, {maximumFractionDigits:2})}`:''}
            ...
            <IonActionSheet
                mode='ios'
                isOpen={isActionMotor}
                header='Besaran CC'
                onDidDismiss={()=>{setIsActionMotor(false)}}
                buttons={[
                    {
                        text: '< 150.CC',
                        handler() {
